...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/11 19:32:24 by cshinoha          #+#    #+#             */
/*   Updated: 2021/02/11 19:33:14 by cshinoha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				parse_types_code(t_cursor *cursor)
{
	...
	if (cursor->op->args_types_code)
	{
		...
		if (cursor->op->args_num >= 1)
			cursor->args_types[0] = g_arg_code[
										((args_types_code & 0xC0) >> 6) - 1];
		if (cursor->op->args_num >= 2)
			cursor->args_types[1] = g_arg_code[
										((args_types_code & 0x30) >> 4) - 1];
		if (cursor->op->args_num >= 3)
			cursor->args_types[2] = g_arg_code[
										((args_types_code & 0xC) >> 2) - 1];
	}
	else
